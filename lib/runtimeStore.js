...
const VIOLATION_WINDOW_MS =
  (Number(process.env.VIOLATION_WINDOW_MINUTES) || 10) * 60 * 1000;
const VIOLATION_THRESHOLD = Number(process.env.VIOLATION_THRESHOLD) || 5;
const BLOCK_DURATION_MS =
  (Number(process.env.BLOCK_DURATION_HOURS) || 1) * 60 * 60 * 1000;
...
function deserializePayload(payloadEncrypted) {
  ...
  const encryptedValue =
    typeof payloadEncrypted === "string"
      ? payloadEncrypted
...
async function savePendingOnboarding(telegramId, chatId, attempts = 0) {
  let response = await supabase.from("pending_onboarding_vault").upsert(
    {
  ...
  if (isMissingRelationError(response.error)) {
    response = await supabase.from("bot_pending_onboarding").upsert(
      {
        telegram_id: normalizeTelegramId(telegramId),
        chat_id: String(chatId),
        attempts,
...
async function getPendingAction(telegramId) {
  ...
  if (isMissingRelationError(response.error)) {
    response = await supabase
      ...
      .select(
        "telegram_id, chat_id, action_type, payload_encrypted, expires_at",
      )
      .eq("telegram_id", normalizeTelegramId(telegramId))
      .maybeSingle();
...
async function savePendingAction(
  ...
  if (
    isMissingRelationError(response.error) ||
    isMissingColumnError(response.error)
  ) {
    response = await supabase.from("bot_pending_actions").upsert(
      {
        telegram_id: normalizeTelegramId(telegramId),
        chat_id: String(chatId),
        action_type: actionType,
...
async function isPermanentlyBlocked(telegramId) {
  ...
  try {
    ...
    logWarn("isPermanentlyBlocked DB check failed, failing open", {
      telegramId,
      error: err.message,
    });
    return false;
...
async function pruneRuntimeState() {
  ...
  const staleWindowIso = new Date(
    Date.now() - VIOLATION_WINDOW_MS * 2,
  ).toISOString();
