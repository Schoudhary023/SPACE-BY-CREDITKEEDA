...
async function fetchExpiringCards() {
  ...
  const { data: cards, error } = await withSupabaseRetry(() =>
    supabase
      ...
      .select("id, telegram_id, brand, amount, expiry_date")
  ...
  const remindedCardIds = new Set(
    (reminders || []).map((row) => Number(row.gift_card_id)),
  );
...
async function saveReminderRows(cards) {
  ...
  const { error } = await withSupabaseRetry(() =>
    supabase.from("expiry_reminders_vault").upsert(
      cards.map((card) => ({
...
async function runExpiryReminderJob(bot) {
  ...
  if (!cards.length) {
    logInfo("Expiry reminder job completed", {
      notifiedUsers: 0,
      notifiedCards: 0,
    });
    return;
  ...
  for (const [telegramId, userCards] of grouped.entries()) {
    try {
      await bot.sendMessage(
        Number(telegramId),
        buildReminderMessage(userCards),
        {
      ...
      track("expiry.reminder_sent", telegramId, {
