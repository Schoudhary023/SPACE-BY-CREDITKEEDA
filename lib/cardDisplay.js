...
function getBrandEmoji(brand) {
  ...
  // Normalize brand aliases before matching so display stays consistent across
  // user-entered text, forwarded messages, and AI-extracted cards.
  const brandRules = [
...
function buildListIntro(
  cards,
  { includeRedeemed, searchTerm, showing, total },
) {
...
function formatIsoDateInTimeZone(date, timeZone) {
  // Expiry comparisons use calendar days in the configured reminder timezone,
