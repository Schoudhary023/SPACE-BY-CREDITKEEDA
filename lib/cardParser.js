...
function parseMissingCardDetailsInput(text, { needsAmount = false, needsExpiry = false } = {}) {
...
const INVALID_BRAND_PATTERN = /^(?:inr|rs|rupees?|amount|value|code|pin|voucher|gift|card|giftcard|evoucher|e-voucher)$/i;
...
function parseCardInput(text) {
  ...
  for (let width = 3; width >= 1 && !expiryDate; width -= 1) {
    for (let index = rest.length - width; index >= 0; index -= 1) {
      const normalizedExpiry = normalizeExpiryDate(rest.slice(index, index + width).join(" "));
      if (normalizedExpiry) {
...
function sanitizeParsedCard(parsed, options = {}) {
  ...
  const amountMissing = parsed.amount == null || String(parsed.amount).trim() === "";
  const amount = amountMissing ? null : Number(parsed.amount);
  const hasValidAmount = Number.isFinite(amount) && amount > 0;
  const expiryDate = parsed.expiryDate ? normalizeExpiryDate(parsed.expiryDate) : null;
