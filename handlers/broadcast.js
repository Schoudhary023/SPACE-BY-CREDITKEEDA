...
const BROADCAST_BATCH_DELAY_MS = positiveIntegerEnv(
...
// Telegram returns retry_after when a broadcast hits flood limits. We honor it
// per recipient so one slow send does not stop the whole batch.
...
async function handleBroadcast(bot, msg, text, options = {}) {
  ...
  await bot.sendMessage(
    adminChatId,
    `📢 Broadcasting to ${telegramIds.length} users in batches of ${BROADCAST_BATCH_SIZE}...`,
  );
  ...
  // Send in small batches to avoid Telegram flood limits and to keep progress
  // visible to the admin for long broadcasts.
  for (let offset = 0; offset < total; offset += BROADCAST_BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map((telegramId) =>
        sendBroadcastMessage(bot, telegramId, text, options),
      ),
    );
    ...
    if (processed < total) {
      await bot
        .sendMessage(
          adminChatId,
          `⏳ Progress: ${processed}/${total} (${sent} sent, ${failed} failed)`,
        )
        .catch(() => {});
