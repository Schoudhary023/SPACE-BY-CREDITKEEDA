...
const { hashUserId, track } = require("../lib/analytics");
const {
  ERROR,
  FOLDER,
  IMAGE_MIME_PREFIX,
...
async function logCardEvent(telegramId, cardId, eventType, metadata = {}) {
  ...
  if (error) {
    logWarn("Failed to write card event", {
      telegramId,
      cardId,
      eventType,
      error: error.message,
...
function cleanupExpiredMapEntries() {
  ...
  for (const [key, entry] of renderedListStates.entries()) {
    if (entry._expiresAt && entry._expiresAt <= now)
      renderedListStates.delete(key);
  }

  for (const [key, entry] of pendingSaveConfirmations.entries()) {
    if (entry._expiresAt && entry._expiresAt <= now)
      pendingSaveConfirmations.delete(key);
  }
...
function isImageMessage(msg) {
  return Boolean(
    ...
    (msg.document &&
      ...
      msg.document.mime_type.startsWith(IMAGE_MIME_PREFIX)),
  );
}
...
function setRenderedListState(telegramId, chatId, state) {
  renderedListStates.set(getRenderedListKey(telegramId, chatId), {
    ...state,
...
async function deleteMessages(bot, chatId, messageIds = []) {
  for (const messageId of messageIds) {
    try {
      ...
      logWarn("Failed to delete rendered message", {
        chatId,
        messageId,
        error: error.message,
      });
...
async function requestMissingDetails(
  bot,
  chatId,
  telegramId,
  parsedCards,
...
async function startProcessingIndicator(bot, chatId, text) {
  ...
  const sendAction = () => {
    bot
      ...
      .catch((err) => logWarn("sendChatAction failed", { error: err.message }));
  };
  ...
  try {
    ...
    logWarn("Failed to send processing indicator", {
      chatId,
      error: err.message,
    });
  }
    ...
    async stop() {
      ...
      if (messageId) {
        await bot.deleteMessage(chatId, messageId).catch((err) =>
          logWarn("Failed to delete processing indicator", {
            chatId,
            error: err.message,
          }),
        );
...
function pickPreferredTelegramPhoto(photos) {
  ...
  const withDimensions = photos.filter(
  ...
  const preferred = [...withDimensions]
    .reverse()
    .find(
      (photo) =>
        Math.max(Number(photo.width), Number(photo.height)) >=
        PREFERRED_PHOTO_MIN_DIMENSION,
    );
...
async function downloadTelegramImagePayload(bot, msg) {
  ...
  const document =
    msg.document && msg.document.mime_type?.startsWith(IMAGE_MIME_PREFIX)
      ? msg.document
      : null;
...
async function parseCardsFromImage(bot, msg, telegramId) {
  ...
  const vertexCards = sanitizeParsedCards(raw, {
    minConfidence: 0.6,
    requireAmount: false,
  });
...
async function parseCardFromText(msg, telegramId) {
  ...
  if (isAiParsingEnabled()) {
    ...
    track("card.parse.text", telegramId, {
      ...
      confidence:
        typeof vertexRaw?.confidence === "number" ? vertexRaw.confidence : null,
  ...
  track("card.parse.text", telegramId, {
    mode: "failed",
    success: false,
    reason: "ai_disabled_or_unconfigured",
  });
  return { card: null, mode: "failed" };
}
...
async function fetchCards(
  telegramId,
  { includeRedeemed = false, searchTerm = null } = {},
) {
  let query = supabase
    .from("gift_cards_vault")
    .select(
      "id, brand, amount, code_encrypted, pin_encrypted, expiry_date, is_redeemed, created_at",
    )
...
async function decryptCardSecrets(card, vaultPin, encryptionSalt) {
  ...
  const decryptedPin = card.pin_encrypted
    ? await decrypt(card.pin_encrypted, vaultPin, encryptionSalt)
    : null;
...
async function sendCards(
  ...
  const decryptResults = await Promise.allSettled(
    cards.map((card) =>
      decryptCardSecrets(card, vaultPin, encryptionSalt).then((secrets) => ({
        card,
        ...secrets,
      })),
    ),
  ...
  if (failedIndex !== -1) {
    logError("Card decryption error", decryptResults[failedIndex].reason, {
      telegramId,
      cardId: cards[failedIndex].id,
    });
    await bot.sendMessage(
      chatId,
      `${ERROR} Could not decrypt\\. Make sure you entered the correct vault PIN\\.`,
      {
  ...
  for (let i = 0; i < decryptedCards.length; i++) {
    ...
    const sent = await bot.sendMessage(
      chatId,
      cardMessage(card, i + 1, decryptedCode, decryptedPin),
      {
        parse_mode: "MarkdownV2",
...
async function sendCardListFresh(
  bot,
  chatId,
  telegramId,
  vaultPin,
  encryptionSalt,
  allCards,
  { includeRedeemed, searchTerm },
) {
  const visibleCards = allCards.slice(0, PAGE_SIZE);
  const messageIds = [];

  if (visibleCards.length) {
    const intro = await bot.sendMessage(
      chatId,
      buildListIntro(visibleCards, {
        includeRedeemed,
        searchTerm,
  ...
  const showMoreRow = buildShowMoreRow(visibleCards.length, allCards.length);
  ...
  const showMoreMsgId = showMoreRow
    ? result.messageIds[result.messageIds.length - 1]
    : null;
  setRenderedListState(telegramId, chatId, {
    includeRedeemed,
    searchTerm,
    messageIds,
    allCards,
    offset: visibleCards.length,
    showMoreMsgId,
    lastCardBaseRows: result.lastCardBaseRows,
  });
...
async function editCardListInPlace(
  bot,
  chatId,
  telegramId,
  vaultPin,
  ...
  const decryptResults = await Promise.allSettled(
    visibleCards.map((card) =>
      decryptCardSecrets(card, vaultPin, encryptionSalt).then((s) => ({
        card,
        ...s,
      })),
    ),
  ...
  if (failedIndex !== -1) {
    logError(
      "Card decryption error during in-place edit",
      decryptResults[failedIndex].reason,
      { telegramId, cardId: visibleCards[failedIndex].id },
    );
    await bot.sendMessage(
      chatId,
      `${ERROR} Could not decrypt\\. Make sure you entered the correct vault PIN\\.`,
  ...
  try {
    ...
    if (isMessageNotModifiedError(error)) {
      ...
      logWarn("Edit-in-place intro failed, falling back to fresh render", {
        chatId,
        error: error.message,
      });
      await clearRenderedListState(bot, telegramId, chatId);
      return sendCardListFresh(
        bot,
        chatId,
        telegramId,
        vaultPin,
        ...
        try {
          await bot.editMessageText(
            cardMessage(card, i + 1, decryptedCode, decryptedPin),
            {
              ...
              parse_mode: "MarkdownV2",
              reply_markup: {
                inline_keyboard: buildCardKeyboard(
                  card,
                  decryptedCode,
                  decryptedPin,
                ),
          ...
          logWarn("Failed to edit card message in-place, will send new", {
            chatId,
            messageId: existingMsgId,
            error: error.message,
          });
          pendingNewSends.push({
            i,
            card,
            decryptedCode,
            decryptedPin,
            staleId: existingMsgId,
          });
          return { i, msgId: null };
        }
      } else {
  ...
  for (const {
    ...
    decryptedPin,
    staleId,
  } of pendingNewSends.sort((a, b) => a.i - b.i)) {
    const sent = await bot.sendMessage(
      chatId,
      cardMessage(card, i + 1, decryptedCode, decryptedPin),
      {
        parse_mode: "MarkdownV2",
        reply_markup: {
          inline_keyboard: buildCardKeyboard(card, decryptedCode, decryptedPin),
        },
      },
    );
    msgIdMap.set(i, sent.message_id);
    if (staleId) {
      bot.deleteMessage(chatId, staleId).catch((err) =>
        logWarn("Failed to delete stale card message", {
          chatId,
          messageId: staleId,
          error: err.message,
        }),
  ...
  await deleteMessages(
    bot,
    chatId,
    existingCardMessageIds.slice(visibleCards.length),
  );

  // Add/remove show_more button on the last card
  const showMoreRow = buildShowMoreRow(visibleCards.length, allCards.length);
  const lastDecrypted = decryptedCards[decryptedCards.length - 1];
  const lastCardBaseRows = buildCardKeyboard(
    lastDecrypted.card,
    lastDecrypted.decryptedCode,
    lastDecrypted.decryptedPin,
  );
  const lastCardMsgId = newMessageIds[newMessageIds.length - 1];
    ...
    await bot
      ...
      .catch((err) => {
        if (!isMessageNotModifiedError(err)) {
          logWarn("Failed to add show_more to last card", {
            chatId,
            error: err.message,
          });
        }
...
async function sendCardList(
  ...
  const allCards =
    prefetchedCards ??
    (await fetchCards(telegramId, { includeRedeemed, searchTerm }));

  if (
    !forceRefresh &&
    existingState?.messageIds?.length > 0 &&
    allCards.length > 0
  ) {
    return editCardListInPlace(
      bot,
      chatId,
      telegramId,
      vaultPin,
...
async function findDuplicateCard(
  ...
  for (const card of existingCards) {
    ...
    try {
      ...
      logError("Duplicate card check decryption error", error, {
        telegramId,
        cardId: card.id,
      });
    }
...
async function doSaveCard(
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    await bot.sendMessage(
      chatId,
      `${WARNING} Amount is required before saving this gift card\\.`,
      {
        parse_mode: "MarkdownV2",
      },
    );
  ...
  const pinEncrypted = parsed.pin
    ? await encrypt(parsed.pin, vaultPin, encryptionSalt)
    : null;
  ...
  await logCardEvent(telegramId, inserted.id, "saved", {
    brand: parsed.brand,
    amount,
  });
  track("card.saved", telegramId, { brand: parsed.brand, amount, source });
  await bot.sendMessage(
    chatId,
...
async function sendSaveConfirmation(
  bot,
  chatId,
  telegramId,
  parsedCards,
  ...
  if (existingPending?.promptMessageId) {
    try {
      await bot.deleteMessage(
        existingPending.chatId,
        existingPending.promptMessageId,
      );
    } catch (_) {}
  }
...
async function handlePendingDetailsResponse(bot, msg, telegramId) {
  ...
  pendingDetails.parsedCards[missingIndex] = {
    ...
    _expirySkipped:
      parsedDetails.skippedExpiry || currentCard._expirySkipped || false,
  };
  ...
  try {
    ...
    if (isImageMessage(msg)) {
      pendingDetailRequests.delete(
        getPendingDetailKey(telegramId, msg.chat.id),
      );
      ...
      let parseResult;
      const indicator = await startProcessingIndicator(
        bot,
        msg.chat.id,
        `${FOLDER} Reading your gift card image...`,
      );
      try {
        parseResult = await parseCardsFromImage(bot, msg, telegramId);
      } catch (error) {
        if (isVertexFallbackError(error)) {
          logWarn("Vertex temporarily unavailable during image save", {
            telegramId,
            code: error.code,
          });
          await bot.sendMessage(msg.chat.id, buildPlainTextFallbackMessage(), {
            parse_mode: "MarkdownV2",
          });
          return;
        }
      ...
      if (
        ...
        track("card.save_image_details_requested", telegramId, {
          ...
          missing_amount_count: newCards.filter(
            (card) => getMissingDetails(card).needsAmount,
          ).length,
          missing_expiry_count: newCards.filter(
            (card) => getMissingDetails(card).needsExpiry,
          ).length,
        });
        return;
      }
    ...
    try {
      ...
      if (isVertexFallbackError(error)) {
        logWarn("Vertex temporarily unavailable during text save", {
          telegramId,
          code: error.code,
        });
        await bot.sendMessage(msg.chat.id, buildPlainTextFallbackMessage(), {
    ...
    const duplicateCard = await findDuplicateCard(
      bot,
      telegramId,
      vaultPin,
      encryptionSalt,
    ...
    await doSaveCard(
      ...
      encryptionSalt,
      parsed,
      `text_${parseResult.mode}`,
    );
  } catch (error) {
    track("error.handler", telegramId, {
      handler: "save_card",
...
async function handleShow(bot, msg, session) {
  ...
  try {
    ...
    const { cards, sent } = await sendCardList(
      bot,
      msg.chat.id,
      telegramId,
      vaultPin,
      encryptionSalt,
      { includeRedeemed: false, forceRefresh: true, prefetchedCards },
    );
    ...
    if (!sent && !cards.length) {
      await bot.sendMessage(
        msg.chat.id,
        `${FOLDER} No active gift cards\\. Send a card image or use /show\\_all to include redeemed cards\\.`,
        {
          parse_mode: "MarkdownV2",
        },
      );
...
async function handleShowAll(bot, msg, session) {
  ...
  try {
    ...
    const { cards, sent } = await sendCardList(
      ...
      vaultPin,
      encryptionSalt,
      { includeRedeemed: true, forceRefresh: true, prefetchedCards },
    );
    ...
    if (!sent && !cards.length) {
      await bot.sendMessage(
        msg.chat.id,
        `${FOLDER} No gift cards saved yet\\. Send a card image or text to get started\\.`,
        {
          parse_mode: "MarkdownV2",
        },
      );
...
async function handleSearch(bot, msg, session, searchTerm) {
  ...
  try {
    ...
    bot
      .sendChatAction(msg.chat.id, "typing")
      .catch((err) => logWarn("sendChatAction failed", { error: err.message }));
    const prefetchedCards = await fetchCards(telegramId, {
    ...
    track("card.searched", telegramId, {
...
async function handleCallback(bot, callbackQuery, session) {
  ...
  try {
    const actionsRequiringCardId = new Set([
      "delete",
      "confirm_delete",
      "cancel_delete",
      "mark_redeemed",
    ...
    if (
      !action ||
      (actionsRequiringCardId.has(action) &&
        (!Number.isInteger(cardId) || cardId <= 0))
    ) {
      await bot.answerCallbackQuery(callbackQuery.id, {
        text: `${ERROR} Invalid action`,
      });
      return;
    }

    if (action === "show_more") {
      const state = getRenderedListState(
        telegramId,
      ...
      const freshAllCards = await fetchCards(telegramId, {
        includeRedeemed: state.includeRedeemed,
      ...
      const failedIndex = decryptResults.findIndex(
        (r) => r.status === "rejected",
      );
      if (failedIndex !== -1) {
        logError(
          "Card decryption error during show_more",
          decryptResults[failedIndex].reason,
          { telegramId, cardId: nextBatch[failedIndex].id },
        );
        await bot.answerCallbackQuery(callbackQuery.id, {
          text: `${ERROR} Could not decrypt. Check your vault PIN.`,
        });
        return;
      }
        ...
        await bot
          .editMessageReplyMarkup(
            { inline_keyboard: state.lastCardBaseRows },
            {
              ...
              message_id: state.showMoreMsgId,
            },
          )
          .catch(() => {});
      }
      ...
      for (let i = 0; i < decryptedBatch.length; i++) {
        ...
        const rows =
          isLast && newShowMoreRow ? [...baseRows, newShowMoreRow] : baseRows;
        const sent = await bot.sendMessage(
          showMoreChatId,
          cardMessage(card, globalOffset + i + 1, decryptedCode, decryptedPin),
          {
            parse_mode: "MarkdownV2",
            reply_markup: { inline_keyboard: rows },
          },
      ...
      const newShowMoreMsgId = newShowMoreRow
        ? newMessageIds[newMessageIds.length - 1]
        : null;
      setRenderedListState(telegramId, showMoreChatId, {
        ...state,
        allCards: freshAllCards,
        messageIds: newMessageIds,
        offset: nextOffset,
        showMoreMsgId: newShowMoreMsgId,
        lastCardBaseRows: newLastCardBaseRows,
      });
      return;
    }

    if (action === "confirm_save") {
      const pendingSaveKey = getPendingSaveKey(
        telegramId,
        callbackQuery.message.chat.id,
      );
      ...
      if (!pendingSave) {
        await bot.answerCallbackQuery(callbackQuery.id, {
          text: `${ERROR} No pending save found. Try sending the image again.`,
        });
        return;
      }
      ...
      try {
        await bot.deleteMessage(
          pendingSave.chatId,
          pendingSave.promptMessageId,
        );
      } catch (deleteError) {
        logWarn("Failed to delete save confirmation prompt", {
          telegramId,
    ...
    if (action === "cancel_save") {
      ...
      const pendingSave = pendingSaveConfirmations.get(pendingSaveKey);
      pendingSaveConfirmations.delete(pendingSaveKey);
      track("card.save_cancelled", telegramId, {
        card_count: pendingSave?.parsedCards?.length ?? 0,
      });
      ...
      if (pendingSave?.promptMessageId) {
        try {
          ...
          logWarn("Failed to delete save confirmation prompt on cancel", {
            telegramId,
            error: deleteError.message,
          });
        }
    ...
    if (action === "delete") {
      ...
      await bot.answerCallbackQuery(callbackQuery.id);
      const sentConfirmation = await bot.sendMessage(
        callbackQuery.message.chat.id,
        confirmation.text,
        {
          reply_markup: confirmation.reply_markup,
      ...
      pendingDeleteConfirmations.set(getPendingDeleteKey(telegramId, cardId), {
        ...
        brand: card.brand,
        listState: getRenderedListState(
          telegramId,
          callbackQuery.message.chat.id,
        ),
        _expiresAt: Date.now() + CONFIRMATION_TTL_MS,
      ...
      if (pendingDelete?.chatId && pendingDelete?.promptMessageId) {
        try {
          await bot.deleteMessage(
            ...
            pendingDelete.promptMessageId,
          );
        } catch (deleteError) {
          logWarn("Failed to delete cancellation prompt", {
            telegramId,
            cardId,
            error: deleteError.message,
          });
    ...
    if (action === "confirm_delete") {
      ...
      const deletedLabel = pendingDelete
        ? `${getBrandEmoji(pendingDelete.brand)} ${pendingDelete.brand}`
        : `${getBrandEmoji(card.brand)} ${card.brand}`;
      const listState = pendingDelete?.listState ||
        getRenderedListState(telegramId, callbackQuery.message.chat.id) || {
          includeRedeemed: false,
          searchTerm: null,
        };
      ...
      if (pendingDelete?.promptMessageId) {
        try {
          ...
          logWarn("Failed to delete confirmation prompt", {
            telegramId,
            cardId,
            error: deleteError.message,
          });
      ...
      const refreshedList = await sendCardList(
        bot,
        callbackQuery.message.chat.id,
        telegramId,
        vaultPin,
      ...
      if (!refreshedList.sent && !refreshedList.cards.length) {
        await bot.sendMessage(
          callbackQuery.message.chat.id,
          getEmptyStateMessage(listState),
          {
            parse_mode: "MarkdownV2",
          },
        );
    ...
    if (action === "confirm_delete_account") {
      ...
      await clearRenderedListState(
        bot,
        telegramId,
        callbackQuery.message.chat.id,
      );
      await bot.answerCallbackQuery(callbackQuery.id, {
        text: `${TRASH} Account deleted`,
      });
      await bot.editMessageText(
        `${TRASH} Your account and all data have been permanently deleted\\.`,
        {
          chat_id: callbackQuery.message.chat.id,
    ...
    if (action === "confirm_delete_all") {
      ...
      await bot.answerCallbackQuery(callbackQuery.id, {
        text: `${TRASH} All gift cards deleted`,
      });
    ...
    if (action === "mark_redeemed") {
      ...
      logInfo("Card redeemed", {
        telegramId,
        cardId,
        brand: card.brand,
        redeemedAt,
      });
      await logCardEvent(telegramId, cardId, "redeemed", {
        brand: card.brand,
        amount: card.amount,
        redeemedAt,
      });

      const listState = getRenderedListState(
        telegramId,
        callbackQuery.message.chat.id,
      ) || { includeRedeemed: false, searchTerm: null };
      track("card.marked_redeemed", telegramId, { brand: card.brand });
      await bot.answerCallbackQuery(callbackQuery.id, {
        text: `${SUCCESS} Marked as redeemed`,
      });
      const refreshedList = await sendCardList(
        bot,
        callbackQuery.message.chat.id,
