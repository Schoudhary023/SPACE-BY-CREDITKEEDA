...
const BLOCK_DURATION_LABEL =
  BLOCK_DURATION_HOURS === 1 ? "1 hour" : `${BLOCK_DURATION_HOURS} hours`;
...
if (
  ...
  String(process.env.AI_PARSING_ENABLED || "").toLowerCase() !== "true"
) {
  logWarn(
    "Vertex API key is configured, but AI parsing remains disabled until AI_PARSING_ENABLED=true",
  );
}
...
function isSaveCandidateMessage(msg) {
  return Boolean(
    isImageMessage(msg) || (msg.text && !msg.text.trim().startsWith("/")),
  );
}
...
bot.on("message", async (msg) => {
  ...
  const { command, args, rawArgs, rawArgsOffset } = parseCommand(
    msg.text || "",
  );
  ...
  await withErrorReply(bot, chatId, async () => {
    if (!isPrivateChat(msg.chat)) {
      if (
        ...
        command === "/end" ||
        isSaveCandidateMessage(msg)
      ) {
        await bot.sendMessage(
          chatId,
          `${WARNING} For privacy, use this bot only in a private chat\\.`,
          {
            parse_mode: "MarkdownV2",
          },
        );
    ...
    if (
      ...
      if (
        ...
        if (command === "/broadcast") {
          ...
          await bot.sendMessage(
            chatId,
            `✅ User ${targetId} permanently blocked.${reasonNote}`,
          );
    ...
    if (
      (await isBlocked(telegramId)) ||
      (await isPermanentlyBlocked(telegramId))
    ) {
      return;
    }
    ...
    if (command === "/end") {
      ...
      await bot.sendMessage(
        chatId,
        `${LOCK} Session ended\\. Type /start to begin again\\.`,
        {
          parse_mode: "MarkdownV2",
        },
      );
    ...
    if (!generalLimit.allowed) {
      ...
      track("error.rate_limit", telegramId, {
        limit_type: "general",
        blocked: justBlocked,
      });
      if (justBlocked) {
        await bot.sendMessage(
          chatId,
          `${WARNING} You have been blocked for ${BLOCK_DURATION_LABEL} due to repeated abuse\\.`,
          { parse_mode: "MarkdownV2" },
        );
        return;
      }
    ...
    if (await isAwaitingOnboarding(telegramId)) {
      if (!msg.text) {
        await bot.sendMessage(
          chatId,
          `${ERROR} Send a PIN with at least 6 digits to finish setup\\.`,
          {
            parse_mode: "MarkdownV2",
          },
        );
    ...
    if (await isAwaitingPin(telegramId)) {
      if (!msg.text) {
        await bot.sendMessage(
          chatId,
          `${ERROR} Enter your vault PIN to continue\\.`,
          {
            parse_mode: "MarkdownV2",
          },
        );
    ...
    if (
      ...
      await bot.sendMessage(
        chatId,
        `${WARNING} Your active vault session is bound to a different chat\\. Use /end there or continue in that original private chat\\.`,
        {
          parse_mode: "MarkdownV2",
        },
      );
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  try {
    ...
    if (!generalLimit.allowed) {
      ...
      if (justBlocked) {
        await bot.answerCallbackQuery(callbackQuery.id, {
          text: `${WARNING} Blocked for ${BLOCK_DURATION_LABEL} due to repeated abuse.`,
        });
        return;
      }
      await bot.answerCallbackQuery(callbackQuery.id, {
...
logInfo("Space by Creditkeeda bot is running", {
